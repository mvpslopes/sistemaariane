...
import { LayoutDashboard, PawPrint, FileText, Banknote, UserCircle } from 'lucide-react';
import { clientPortalLabels } from '../constants/clientPortalLabels';

const items = [
  { to: '/app', end: true, icon: LayoutDashboard, label: 'Início' },
  { to: '/app/animais', end: false, icon: PawPrint, label: 'Compras' },
  { to: '/app/contratos', end: false, icon: FileText, label: 'Contratos' },
  { to: '/app/cobrancas', end: false, icon: Banknote, label: 'Cobranças' },
  { to: '/app/perfil', end: false, icon: UserCircle, label: 'Perfil' },
] as const;
        ...
        {items.map(({ to, end, icon: Icon, label }) => (
          <li key={to} className="flex-1">
            <NavLink
              ...
              title={to === '/app/animais' ? clientPortalLabels.animalsNav : label}
